        ...
        while($reg = mysqli_fetch_assoc($r)){
			...
			echo '<tr class="bg-danger text text-white" style="border:none">
                ...
                <td style="border:none" ><strong> DIA ',  date('d/m/Y', strtotime($reg['data_fim'])); 
				 $clientes = mysqli_query($conn,'SELECT * FROM instalacoes WHERE data_fim = "'. $reg['data_fim'] .'" and concluido="SIM" ORDER BY nome DESC;');
				$row_cnt = mysqli_num_rows($clientes);
...
<td style="border:none"></td>
 ...
 data-whateverobservacao="" 
 data-whateverconclusao=""    
  data-whateverbairro="'.$cliente['bairro'].'"   
   data-whateverrua="'.$cliente['rua'].'"   
    ...
    data-whatevertecnico="'.$cliente['tecnico'].'"   
     ...
     data-whateverplano=""                 
       data-whateveroperador="" 
       data-whateversituacao="" 
       data-whateverdata-cadastro=""                     
       data-whateveralterado_por="'.$cliente['alterado_por'].'" 
       data-whateverultima_alteracao="'.date('d/m/Y - H:i:s', strtotime($cliente['ultima_alteracao'])).'"> <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-clipboard2-check" viewBox="0 0 16 16">
...
*CABO DROP:* '.$cliente['cabo_drop'].'
*ESTICADORES:* '.$cliente['esticador'].'
*CONECTORES:* '.$cliente['conector'].'
*BUXA/PARAFUSO:* '.$cliente['buxa_parafuso'].'
*PRENSA FIO:* '.$cliente['prensa'].'
